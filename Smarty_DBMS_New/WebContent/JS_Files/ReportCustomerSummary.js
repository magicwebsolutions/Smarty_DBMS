...
function onload() {
	...
	document.getElementById("Start_Date").value = yyyy + '-' + mm + '-'
			+ StartDateNumber;
	document.getElementById("End_Date").value = yyyy + '-' + mm + '-'
			+ EndDateNumber;
	getSummary(yyyy + '-' + mm + '-' + StartDateNumber, yyyy + '-' + mm + '-'
			+ EndDateNumber);
}
...
function getCustomerDetailedReport(customerid) {
	...
	if (customerid == "CLEAR") {
		...
		var getCustomerDtlsURL = gloablContextURL
				+ "/Report?Event=GET_CUSTOMER_REPORT&customerId=" + customerid;
		if (window.XMLHttpRequest) {
			requestOBJ = new XMLHttpRequest();
		} else if (window.ActiveXObject) {
			requestOBJ = new ActivXCObject("Microsoft.XMLHTTP");
			...
			if (responsetext.indexOf("~") > 0) {
				...
				if (dataSet.length > 0) {
					...
					$.each(dataSet, function() {
						rows += "<tr class='item'><td>" + this.BillID
								+ "</td><td>" + this.BillDate + "</td><td>"
								+ this.Name + "</td><td>" + this.BillType
								+ "</td><td>" + this.BillAmount + "</td><td>"

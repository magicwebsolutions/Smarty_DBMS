...
function onload() {
	...
	yyyy = parseInt(yyyy);
	today = yyyy + '-' + mm + '-' + dd;
	document.getElementById("stk_EditDtls").className += " button_disable";
	document.getElementById("stk_EditDtls").disabled = true;
	mm = ('0'+mm+'').slice(-2);
	dd = ('0'+dd+'').slice(-2);
	formattedDate = yyyy + '-' + mm + '-' + dd;
	document.getElementById("StockIn_date").value = formattedDate;
	document.getElementById("Add_Stock_date").value = formattedDate;	
}
			...
			if (responsetext.indexOf("~") > 0) {
				...
				for (var i = 0; i < cells.length; i++) {
					...
					cell.onclick = function() {
						...
						var element = document.getElementById("stk_EditDtls");
						element.classList.remove("button_disable");
						document.getElementById("stk_EditDtls").disabled = false;
